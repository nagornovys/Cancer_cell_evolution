...
cell <- setRefClass(
    ...
    methods = list(
        ...
        initialize = function(gene_size, id=1, parent=0, c=0, d=0, i=1, m=0,
                              mutden=0, a=0, k=0, E=0, Nmax=Nmax, gene=NULL, pasgene=NULL,
                              posdriver=NULL, pospasngr=NULL, invasion=FALSE, s=0,birthday=0) {
            id <<- id
            parent <<- parent
            c <<- c
            d <<- d
        ...
        updateCell = function(cell1, F) {
          # Apoptosis
            ...
            if (cell1$d < 0) {cell1$d = 0}
                ...
                # Invasion metastasis
                if (!cell1$invasion) {
                cell1$im = cell1$Him # for soft variant ifelse(cell1$Him > 0.9,cell1$Him,0)
            } else {
                cell1$im = 1
            }
          },
    ...
    if (!cell1$invasion) {
      if (cell1$im > 0) {
        if (cell1$im >= random[gtotal + 5]) {
          cell1$invasion = ifelse(cell1$im == 1,TRUE,FALSE)
        } else {
          return(0)
        }
      }
...
# E0 <- 2E-4                     #as.numeric(args[6])          # (1E-1, 1E-2, 1E-3, 1E-4, 1E-5)
# F0 <- 1E0                      #as.numeric(args[7])          # (10^1, 10^2, 10^3, 10^6, 10^9, 10^12)
# m <-  1E-6                       #as.numeric(args[8])           # mutation probability (1*10e-11, 5*10e-11, 1*10e-10, 5*10e-10, 1*10e-9, 5*10e-9, 1*10e-8, 5*10e-8)
...
# k <-  0.1                       #as.numeric(args[12])          # Environmental death probability
# censore_n <- 30000               #as.numeric(args[14])  # Max cell number where the program forcibly stops
# censore_t <- 200                 #as.numeric(args[15])  # Max time where the program forcibly stops
